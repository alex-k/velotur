...
$u=new Users();
if ($_POST['doSearch'] && !$_POST['userLoadedOK']) {
	unset($_POST['_id']);
	$options=array();	
	foreach ($_POST as $k=>$v) if ($v && $k!='userPartnerID') $options[$k]=$v;
	$user=$u->searchObjectsFromDB($options);
	if (is_array($user)) {
		if ( count($user)==1)  {
			$user=current($user);
			$smarty->assign($user->getValues());
			...
			foreach ($user as $k=>$v) {
				$founded_users[$k]=$v->getValues();
				$founded_users[$k]['http_query']=http_build_query($founded_users[$k]);
			}
			$smarty->assign('founded_users',$founded_users);
		}
	}
}
...
if (!empty($_POST) && !empty($_POST[formSubmit])) {
	...
	if (($smarty->_validate_processed==1 && $smarty->_validate_error!=1)) {
		...
		$tour=new Tours($_POST['applyTourID']);
			...
			if ($_POST['tourUserAddEmail1']) $tour->tourUserAddEmail1=$_POST['tourUserAddEmail1'];
			if ($_POST['tourUserAddEmail2']) $tour->tourUserAddEmail2=$_POST['tourUserAddEmail2'];
